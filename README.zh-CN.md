...
第一轮没有历史数据，默认取 x₁ = 0.5。代码采用从 0 开始的索引，递推写成：
...
每轮在 [−1, 1] 内选择 xₜ，随后观察梯度 gₜ，损失为：

$$
f_t(x)=g_tx
$$

OGD 从 x₁ = 0 开始，以固定步长更新：
...
必须先保存本轮 xₜ，再用 gₜ 更新下一轮，确保本轮决策只依赖过去的信息。
决策域的直径为 D = 2，实验满足 |gₜ| ≤ G = 1，所以此步长下
OGD 的静态遗憾上界为 DG√T = 2√T。
...
$$
x_t=-\mathrm{sign}(\sum_{s=1}^{t-1}g_s)
$$

其中符号函数在正数、负数、零处分别取 1、−1、0。
