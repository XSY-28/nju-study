...
第二个小项目由自己编写的 `OGD.py` 整理而来，对应 Obsidian 讲义 7.2。
保留原来的“先记录决策，再做梯度更新和投影”的循环，借助 AI 修正
梯度序列生成、补充 FTL 对照、输入检查、测试与实验入口。

- `src/ogd.py`：`ogd_linear`、`ftl_linear` 返回每轮决策；`linear_regret` 计算静态遗憾。
- `tests/test_ogd.py`：手算例子、400 轮结果、投影边界、在线时序及无效输入。
- `experiments/run_ogd_linear.py`：生成讲义序列并输出两种算法的对比结果。

### 原练习中需要修正的地方

`where` 是 NumPy 函数，应写成 `np.where(...)`，数组没有 `g.where(...)` 方法。
讲义使用奇数轮为正、偶数轮为负的序列，并把第一项改为 `0.5`：
...
原来的 `desicion` 统一改名为 `decisions`；拼写不影响计算，但改后更便于阅读。
...
必须先保存本轮 $x_t$，再用 $g_t$ 更新下一轮。原练习的这一顺序是正确的。
